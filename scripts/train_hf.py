...
from src.models import MODEL_REGISTRY, Gemma2ForCausalLM
from src.models.gemma2.modeling_gemma2 import generate_sequence_scan
...
from src.utils.inference import GenerationCarry
...
def loss_fn(
    model: tp.Union[nnx.Module, Gemma2ForCausalLM],
    batch: tuple[jax.Array, jax.Array],
):
    """Compute the loss for a batch of data."""
    input_ids, labels = batch

    attention_mask = nnx.make_causal_mask(input_ids, dtype=input_ids.dtype)
...
def _create_sharded_model(
    ...
    model = cls(config, mesh=mesh, rngs=rngs, dtype=dtype)
...
@hydra.main(config_path="../configs", config_name="train_hf", version_base="1.2")
def main(cfg: DictConfig):
    ...
    logger.info("Starting HuggingFace language model training...")
    ...
    model_hub_id = cfg["model_hub_id"]
    logger.info(f"Loading model configuration from {model_hub_id}...")

    # Load HF config to get architecture info
    ...
    pprint(f"Model config: {hf_config}")

    log_node_devices_stats(logger)

    # Model instance
    dtype_str = cfg["dtype"]
    logger.info(f"Creating HF model with dtype={dtype_str}...")
    ...
    model: tp.Optional[nnx.Module] = None
    with mesh:
        model = _create_sharded_model(
            model_name=cfg["model_name"],
            config_fn=lambda: hf_config,
            seed=args.seed,
            mesh=mesh,
            dtype=dtype,
        )
    ...
    # Set context length based on model config
    context_length = getattr(hf_config, "max_position_embeddings", 2048)
    logger.info(f"Using context length: {context_length}")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if eval_batch_count > 0 and eval_data_available:
            ...
            try:
                ...
                input_ids = tokenizer(
                    choosen_prompt, return_tensors="jax", padding=True
                )["input_ids"]
                ...
                # Generate using the model-specific generation function
                ...
                logger.warning(f"Could not generate text samples: {e}")
    ...
    with open(artifacts_dir / "config.json", "w") as f:
        json.dump(hf_config.to_dict(), f, indent=4)
    ...
    if args.push_to_hub:
        ...
        upload_folder(
            ...
            commit_message=cfg.get(
                "upload_message", "HuggingFace language model training completed"
            ),
        )
